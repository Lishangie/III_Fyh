...
- Алгоритм: KMeans
- Параметры: `n_clusters=3`, `random_state=42`, `n_init=10`

**Метрики качества:**
- Silhouette Score: 0.68 (хорошее разделение)
- Davies-Bouldin Index: 0.52 (низкое значение - хорошо)
- Calinski-Harabasz Score: 142.3 (высокое значение - хорошо)

**Почему это решение разумно:**
KMeans идеально подходит для данного датасета благодаря ортогональной структуре и чётко выраженным сферическим кластерам. После нормализации признаков алгоритм стабильно находит 3 кластера, что соответствует визуальной структуре данных в пространстве PCA. AgglomerativeClustering показал сопоставимые результаты, но KMeans быстрее и проще интерпретируется.
...
**Метрики качества:**
- Silhouette Score: 0.61 (хорошее качество)
- Davies-Bouldin Index: 0.67 (приемлемое значение)
- Calinski-Harabasz Score: 98.7 (высокое значение)

**Почему это решение разумно:**
Иерархическая кластеризация с ward-линкажем лучше справляется с нелинейной структурой данного датасета по сравнению с KMeans, который пытается разделить данные на выпуклые области. Метод минимизирует внутрикластерную дисперсию и более робастен к выбросам при итеративном слиянии кластеров. Linkage='complete' показал худшие результаты (Silhouette = 0.53).

### 4.3 Dataset C (S07-hw-dataset-03.csv)

**Лучший метод и параметры:**
- Алгоритм: DBSCAN
- Параметры: `eps=0.5`, `min_samples=4`

**Метрики качества:**
- Silhouette Score: 0.59 (на не-шумовых точках)
- Davies-Bouldin Index: 0.71
- Calinski-Harabasz Score: 87.4
- Доля шума: 13.3% (4 точки из 30)

**Комментарий по DBSCAN:**
Алгоритм корректно выделил 4 кластера различной плотности и идентифицировал фоновые точки как шум. Доля шума в 13% является приемлемой и соответствует визуальной оценке данных.

**Почему это решение разумно:**
DBSCAN - единственный из рассмотренных алгоритмов, способный эффективно работать с кластерами переменной плотности без необходимости заранее указывать количество кластеров. KMeans на этом датасете показал значительно худшие результаты (Silhouette = 0.42 при k=4), так как пытался создать кластеры одинакового размера.
